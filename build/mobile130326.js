class Element{element;constructor(e="div",t=[],i=""){let s=document.createElement(e);s.classList.add(t),s.innerHTML=i,this.element=s}restoreHTML(){this.element.innerHTML=""}}const getOrder=()=>{let e=localStorage.getItem("bucket");return e?JSON.parse(e):[]};class BucketElement extends Element{weigth=1;plusButton;minusButton;addToBucketButton;cake=medoviki[0];constructor(){super("div",["bucket_modal"]),this.element.innerHTML=`
    ...
    `,this.form=this.element.querySelector(".order_person_info"),this.radios=document.querySelectorAll('input[name="deliveryType"]'),this.radioPickupText=this.element.querySelector(".radio__text--pickup"),this.inputName=this.element.querySelector(".order_name"),this.inputPhone=this.element.querySelector(".order_phone"),this.closeButton=this.element.querySelector(".close-button"),this.orderButton=this.element.querySelector(".order_cakes"),this.pickupBlock=this.element.querySelector(".pickupBlock"),this.selectCafes=this.element.querySelector(".select-cafes"),this.pickupBlock.addEventListener("click",e=>{e.stopPropagation(),this.selectCafes.classList.toggle("close")}),cafes.forEach(e=>{let t=document.createElement("div");t.classList.add("cafe"),t.textContent=e.name,t.addEventListener("click",t=>{t.stopPropagation(),this.selectPickupCafe(e)}),this.selectCafes.appendChild(t)}),this.closeButton.addEventListener("click",()=>{this.hide()}),this.inputPhone.addEventListener("focus",()=>{this.inputPhone.value||(this.inputPhone.value="+375(")}),this.inputPhone.addEventListener("input",()=>{let e=getDigits(this.inputPhone.value);if(e.startsWith("375")||(e="375"+e),(e=e.slice(0,12)).length>=5){let t=e.slice(3,5);OPERATORS.includes(t)||(e=e.slice(0,3))}this.inputPhone.value=formatPhone(e),this.inputPhone.setSelectionRange(this.inputPhone.value.length,this.inputPhone.value.length)}),this.inputPhone.addEventListener("blur",()=>{let e=getDigits(this.inputPhone.value);12!==e.length&&(this.inputPhone.value="")}),this.form.addEventListener("submit",async e=>{e.preventDefault();let t=new FormData(e.target),i=Object.fromEntries(t.entries());i.pickupPlace=this.pickupCafe;let s=getOrder(),a=s.map(e=>e.price).reduce((e,t)=>e+t,0),n=`Заказ с мобильной версии.
Имя: ${i.name}
Телефон: ${i.phone}
Тип доставки: ${"delivery"===i.delivery?"Доставка":"Самовывоз"}
${"pickup"===i.delivery?`Место самовывоза: ${i.pickupPlace.shortName}`:""}
${s.map((e,t)=>`${t+1}: ${e.cake.name}, Цена: ${"delivery"===i.delivery?e.price:.8*e.price}, Количество: ${e.weight}кг`).join("\n")}
Сумма: ${"delivery"===i.delivery?a:.8*a}
`,l=await fetch(this.api,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:this.chatId,text:n})});if(l.ok){this.orderSuccess(),localStorage.setItem("bucket",JSON.stringify([]));let o=localStorage.getItem("historyOrder");if(o){let r=JSON.parse(o);r.push(s),localStorage.setItem("historyOrder",JSON.stringify(r))}else localStorage.setItem("historyOrder",JSON.stringify([s]))}}),this.form.addEventListener("change",e=>{let t="pickup"===e.target.value;this.pickupBlock.classList.toggle("hidden",!t),this.selectCafes.classList.toggle("close",!t)})}show(){this.element.classList.add("active")}hide(){this.name="",this.inputName.value="",this.phone="",this.inputPhone.value="",this.element.classList.remove("active"),this.element.querySelector(".order_modal_content").classList.remove("message_success")}selectPickupCafe(e){this.pickupCafe=e,this.selectCafes.classList.add("close"),this.radioPickupText.innerText=`Самовывоз(${e.shortName})`}orderSuccess(){this.element.querySelector(".order_modal_content").classList.add("message_success")}}class TabsElement extends Element{constructor(){super("div",["contacts_tabs"],`
      <header>
